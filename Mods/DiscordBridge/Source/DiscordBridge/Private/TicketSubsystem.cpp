...
void UTicketSubsystem::OnInteractionReceived(const TSharedPtr<FJsonObject>& DataObj)
{
	...
	double TypeD = 0.0;
	const bool bHasType = DataObj->TryGetNumberField(TEXT("type"), TypeD);
	const int32 InteractionType = (bHasType
		&& FMath::IsFinite(TypeD) && TypeD >= 0.0 && TypeD <= static_cast<double>(MAX_int32)
	...
	for (const TSharedPtr<FJsonValue>& Entry : *Tickets)
	{
		...
		// legacy double for state files written by older builds.
		{
				...
				&& (AppealIdStr.Len() < 19 || AppealIdStr <= TEXT("9223372036854775807")))
			{
				...
				&& FMath::Fmod(AppealIdLoad, 1.0) == 0.0)
