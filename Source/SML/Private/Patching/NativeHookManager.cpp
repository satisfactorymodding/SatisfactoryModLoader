...
void* FNativeHookManagerInternal::GetHandlerListInternal(const FString& SymbolId) {
	void** ExistingMapEntry = RegisteredListenerMap.Find(SymbolId);
	return ExistingMapEntry ? *ExistingMapEntry : nullptr;
}

void FNativeHookManagerInternal::SetHandlerListInstanceInternal(const FString& SymbolId, void* HandlerList) {
...
FString HookVirtualFunction(void* ConstructorAddress, void* HookFunctionAddress, void** OutTrampolineFunction, const MemberFunctionPointerInfo& SearchInfo, const BootstrapAccessors& Accessors) {
	if (!InstalledConstructorThunks.Contains(ConstructorAddress)) {
		...
		const ConstructorHookThunk NewThunk = Accessors.CreateConstructorHookThunk();
		...
		SML_LOG(LogNativeHookManager, Log, TEXT("Installed constructor thunk on constructor at %llu"), (uint64_t) ConstructorAddress);
	}
	
	ConstructorHookInfoHolder& InfoHolder = InstalledConstructorThunks.FindChecked(ConstructorAddress);
	const MemberFunctionPointerDigestInfo DigestInfo = Accessors.DigestMemberFunctionPointer(SearchInfo);
	...
	if (!InfoHolder.AlreadyHookedFunctions.Contains(SymbolId)) {
		...
		SML_LOG(LogNativeHookManager, Log, TEXT("Hooking virtual function for constructor at %llu with name %s, Member Function Pointer Size: %d"), ConstructorAddress, *SymbolId, SearchInfo.MemberFunctionPointerSize);
		InfoHolder.AlreadyHookedFunctions.Add(SymbolId);
	}
	return SymbolId;
}

SML_API FString FNativeHookManagerInternal::RegisterVirtualHookFunction(const VirtualFunctionOverrideInfo& SearchInfo, void* HookFunctionPointer, void** OutTrampolineFunction) {
	...
	SML_LOG(LogNativeHookManager, Log, TEXT("Hooking virtual function %s of class %s"), *SearchInfo.SymbolSearchName, *SearchInfo.ClassTypeName);
	...
	SML_LOG(LogNativeHookManager, Log, TEXT("Successfully hooked virtual function %s with constructor %s"), *SymbolSearchName, *ConstructorName);
...
SML_API FString FNativeHookManagerInternal::RegisterHookFunction(const FString& SymbolSearchName, void* HookFunctionPointer, void** OutTrampolineFunction) {
	...
	SML_LOG(LogNativeHookManager, Log, TEXT("Hooking symbol with search name %s"), *SymbolSearchName);
	...
	SML_LOG(LogNativeHookManager, Log, TEXT("Successfully hooked normal function %s"), *SymbolId);
	DigestInfo.SymbolName.Free();
	return SymbolId;
}

void FNativeHookManagerInternal::SetupWithAccessors(const BootstrapAccessors& Accessors) {
	BootstrapAccessorsPtr = MakeShareable(new BootstrapAccessors(Accessors));
}
