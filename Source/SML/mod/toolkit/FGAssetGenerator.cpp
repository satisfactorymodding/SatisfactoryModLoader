	...
	if (FunctionNode->IsA<UK2Node_FunctionEntry>() && FunctionObject->HasField(TEXT("Outputs"))) {
		...
		if (Outputs.Num() > 0) {
			...
			for (const TSharedPtr<FJsonValue> Value : Outputs) {
				...
				const FEdGraphPinType& GraphPinType = CreateGraphPinType(ArgumentObject->GetObjectField(TEXT("PinType")).ToSharedRef());
				ReturnNode->CreateUserDefinedPin(*ReturnPinName, GraphPinType, EGPD_Input, false);
			}
...
UK2Node* CreateCustomFunction(UBlueprint* Blueprint, const FString& FunctionName, const TSharedPtr<FJsonObject>& FunctionObject) {
	...
	if (CanPlaceFunctionAsEventJson(FunctionObject) && EventGraph) {
		return FEdGraphSchemaAction_K2NewNode::SpawnNode<UK2Node_CustomEvent>(
			...
			[FunctionName, FunctionObject, FunctionFlags](UK2Node_CustomEvent* NewInstance) {
			NewInstance->bIsEditable = true;
			NewInstance->CustomFunctionName = *FunctionName;
			NewInstance->FunctionFlags |= FunctionFlags;
			UEdGraph* Graph = NewInstance->GetGraph();
			DefineCustomFunctionFromJson(Graph, NewInstance, FunctionObject);
		});
...
void InitializeBlueprint(UBlueprint* Blueprint, const TSharedPtr<FJsonObject>& BlueprintJson, bool bHasDependents) {
	...
	if (WidgetBlueprint != nullptr) {
		...
		if (RootWidget != nullptr) {
			...
			//Copy animations (at least their descriptions for now)
			const TArray<TSharedPtr<FJsonValue>>& AnimationsArray = BlueprintJson->GetArrayField(TEXT("Animations"));
			for (const TSharedPtr<FJsonValue>& AnimationJson : AnimationsArray) {
				const TSharedPtr<FJsonObject>& AnimationObject = AnimationJson->AsObject();
				FString AnimationName = AnimationObject->GetStringField(TEXT("AnimationName"));
				//Remove _INST postfix from the name. No idea where it came from, but it looks like
				//it's not natural, because natively created animations have MovieScene object name == animation name,
				//And MovieScene object names don't seem to contain this postfix
				if (AnimationName.EndsWith(TEXT("_INST")))
					AnimationName = AnimationName.LeftChop(5);
				TSharedPtr<FJsonObject> MovieSceneObject = AnimationObject->GetObjectField(TEXT("MovieScene"));
				UWidgetAnimation* Animation = DeserializeAnimation(WidgetBlueprint, AnimationName, MovieSceneObject);
...
UAnimBlueprint* CreateAnimationBlueprint(UPackage* Package, const FString& Name, UClass* ParentClass, const TSharedRef<FJsonObject>& BlueprintJson) {
	...
	if (BlueprintJson->HasField(TEXT("TargetSkeleton"))) {
		...
		USkeleton* Skeleton = LoadObject<USkeleton>(nullptr, *SkeletonAssetName);
		if (Skeleton != nullptr) {
