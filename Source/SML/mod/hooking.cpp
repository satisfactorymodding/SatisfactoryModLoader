...
#define CHECK_FUNCHOOK_ERR(arg, message) \
	if (arg != FUNCHOOK_ERROR_SUCCESS) SML::shutdownEngine(SML::formatStr(message, functionNameStr.c_str(), TEXT(": "), funchook_error_message(funchook)));
...
void* registerHookFunction(const std::string& functionNameStr, void* hookFunction) {
	...
	if (installedHookMap[functionNameStr] == nullptr) {
		...
		if (gameFunctionPtr == nullptr) {
			SML::shutdownEngine(SML::formatStr(TEXT("Hook target function not found: "), functionNameStr.c_str()));
		}
