...
bool FVersion::ParseVersion(const FString& String, FString& OutFailureReason) {
	FVersion ResultVersion{};
	EVersionComparisonOp ComparisonOp = EVersionComparisonOp::EQUALS;
	FString FailureReason = ParseVersionTemplate(String, ResultVersion, ComparisonOp);
	if (FailureReason.IsEmpty()) {
		if (ComparisonOp != EVersionComparisonOp::EQUALS) {
			FailureReason = TEXT("Unexpected version comparator");
		}
		if (ResultVersion.ContainsSpecialVersionNumbers()) {
			FailureReason = TEXT("Unexpected wildcards in version declaration");
		}
	}
...
bool FVersionComparator::ParseVersionComparator(const FString& String, FString& OutFailureReason) {
	FVersion ResultVersion{};
	EVersionComparisonOp ComparisonOp = EVersionComparisonOp::EQUALS;
	FString FailureReason = ParseVersionTemplate(String, ResultVersion, ComparisonOp);
	if (FailureReason.IsEmpty()) {
		if (ComparisonOp == EVersionComparisonOp::EQUALS || ComparisonOp == EVersionComparisonOp::CARET) {
			...
			if (ResultVersion.Major == SEMVER_VERSION_NUMBER_WILDCARD ||
				...
				FailureReason = TEXT("Unexpected Wildcard inside normal version comparator");
			}
...
bool ParseVersionForHyphenRange(const FString& String, FVersion& Version, FString& OutFailureReason) {
	EVersionComparisonOp ComparisonOp = EVersionComparisonOp::EQUALS;
	const FString ParseFailureReason = ParseVersionTemplate(String, Version, ComparisonOp);
	//Basic version parsing failed, pass failure code
	if (!ParseFailureReason.IsEmpty()) {
		OutFailureReason = ParseFailureReason;
		return false;
	}
	//Hyphen version ranges cannot contain comparison operators
	if (ComparisonOp != EVersionComparisonOp::EQUALS) {
		OutFailureReason = TEXT("Unexpected version comparator inside Hyphen Version Range");
		return false;
	}
	...
	if (Version.Major == SEMVER_VERSION_NUMBER_WILDCARD ||
		...
		OutFailureReason = TEXT("Unexpected wildcard inside Hyphen Version Range");
		return false;
	}
...
bool ParseHyphenVersionRange(const FString& LeftSideString, const FString& RightSideString, TArray<FVersionComparator>& OutComparators, FString& OutFailureReason) {
	//Parse version strings according to hyphen version range rules
	FVersion LeftSideVersion;
	const bool bSuccessLeft = ParseVersionForHyphenRange(LeftSideString, LeftSideVersion, OutFailureReason);
	FVersion RightSideVersion;
	const bool bSuccessRight = ParseVersionForHyphenRange(RightSideString, RightSideVersion, OutFailureReason);
	...
	if (RightSideVersion.Major == SEMVER_VERSION_NUMBER_UNSPECIFIED) {
		...
		OutFailureReason = TEXT("Invalid Major version in right side of Hyphen Version Range");
		return false;
...
bool FVersionComparatorCollection::ParseVersionCollection(const FString& String, FString& OutFailureReason) {
	...
	while (CurrentIndex < String.Len()) {
		...
		if (CurrentChar == TEXT(' ')) {
			...
			if (bHaveHyphenAfterLastString) {
				OutFailureReason = TEXT("Unexpected hyphen");
				return false;
			}
			...
			if (!bIsCurrentlyInString) {
				...
				if (!LastMatchedVersionString.IsEmpty()) {
					if (!CachedLeftSideHyphenRangeString.IsEmpty()) {
						...
						const bool bSuccess = ParseHyphenVersionRange(CachedLeftSideHyphenRangeString, LastMatchedVersionString, ResultArray, OutFailureReason);
						...
						const bool bSuccess = Comparator.ParseVersionComparator(LastMatchedVersionString, OutFailureReason);
	...
	if (ResultArray.Num() == 0) {
		OutFailureReason = TEXT("Version Comparator Collection cannot be empty");
		return false;
	}
...
bool FVersionRange::ParseVersionRange(const FString& String, FString& OutFailureReason) {
	...
	if (CollectionStrings.Num() == 0) {
		OutFailureReason = TEXT("Version range cannot be empty");
		return false;
	}
	...
	for (const FString& CollectionString : CollectionStrings) {
		...
		const bool bSuccess = Collection.ParseVersionCollection(CollectionString, OutFailureReason);
